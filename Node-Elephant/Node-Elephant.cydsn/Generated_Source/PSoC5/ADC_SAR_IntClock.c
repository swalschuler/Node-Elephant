...
void ADC_SAR_IntClock_Start(void) 
{
    ...
    ADC_SAR_IntClock_CLKEN |= ADC_SAR_IntClock_CLKEN_MASK;
	ADC_SAR_IntClock_CLKSTBY |= ADC_SAR_IntClock_CLKSTBY_MASK;
}
...
void ADC_SAR_IntClock_Stop(void) 
{
    ...
    ADC_SAR_IntClock_CLKEN &= (uint8)(~ADC_SAR_IntClock_CLKEN_MASK);
	ADC_SAR_IntClock_CLKSTBY &= (uint8)(~ADC_SAR_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: ADC_SAR_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SAR_IntClock_StopBlock(void) 
{
    if ((ADC_SAR_IntClock_CLKEN & ADC_SAR_IntClock_CLKEN_MASK) != 0u)
    {
...
#if defined(ADC_SAR_IntClock__CFG3)
        CLK_DIST_AMASK = ADC_SAR_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = ADC_SAR_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* ADC_SAR_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(ADC_SAR_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(ADC_SAR_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: ADC_SAR_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SAR_IntClock_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        ADC_SAR_IntClock_CLKSTBY &= (uint8)(~ADC_SAR_IntClock_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: ADC_SAR_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SAR_IntClock_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = ADC_SAR_IntClock_GetSourceRegister();
    uint16 oldDivider = ADC_SAR_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = ADC_SAR_IntClock_CLKEN & ADC_SAR_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(ADC_SAR_IntClock_DIV_PTR, clkDivider);
                ADC_SAR_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                ADC_SAR_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(ADC_SAR_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((ADC_SAR_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(ADC_SAR_IntClock_DIV_PTR, clkDivider);
				ADC_SAR_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
{
    return CY_GET_REG16(ADC_SAR_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: ADC_SAR_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SAR_IntClock_SetModeRegister(uint8 modeBitMask) 
{
    ADC_SAR_IntClock_MOD_SRC |= modeBitMask & (uint8)ADC_SAR_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: ADC_SAR_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SAR_IntClock_ClearModeRegister(uint8 modeBitMask) 
{
    ADC_SAR_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(ADC_SAR_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: ADC_SAR_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_SAR_IntClock_GetModeRegister(void) 
{
    return ADC_SAR_IntClock_MOD_SRC & (uint8)(ADC_SAR_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: ADC_SAR_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SAR_IntClock_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = ADC_SAR_IntClock_GetDividerRegister();
    uint8 oldSrc = ADC_SAR_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        ADC_SAR_IntClock_MOD_SRC |= CYCLK_SSS;
        ADC_SAR_IntClock_MOD_SRC =
            (ADC_SAR_IntClock_MOD_SRC & (uint8)(~ADC_SAR_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: ADC_SAR_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_SAR_IntClock_GetSourceRegister(void) 
{
    return ADC_SAR_IntClock_MOD_SRC & ADC_SAR_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: ADC_SAR_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void ADC_SAR_IntClock_SetPhaseRegister(uint8 clkPhase) 
{
    ADC_SAR_IntClock_PHASE = clkPhase & ADC_SAR_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: ADC_SAR_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 ADC_SAR_IntClock_GetPhaseRegister(void) 
{
    return ADC_SAR_IntClock_PHASE & ADC_SAR_IntClock_PHASE_MASK;
}
