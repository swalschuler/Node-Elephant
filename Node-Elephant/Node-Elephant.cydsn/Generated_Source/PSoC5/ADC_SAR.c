...
void ADC_SAR_Init(void)
{
    ...
    ADC_SAR_tempChan = ADC_SAR_TempBuf_DmaInitialize(ADC_SAR_TEMP_BYTES_PER_BURST,
...
void ADC_SAR_Enable(void)
{
    ...
    */
    ...
    ADC_SAR_CONTROL_REG |= ((uint8)(ADC_SAR_BASE_COMPONENT_ENABLE));
    ADC_SAR_CONTROL_REG |= ((uint8)(ADC_SAR_LOAD_COUNTER_PERIOD));
...
void ADC_SAR_Stop(void)
{
    ...
    *******************************************************************************/
    void ADC_SAR_StartConvert(void)
    {
            ...
            ADC_SAR_CONTROL_REG |= ((uint8)(ADC_SAR_SOFTWARE_SOC_PULSE));
        ...
        #endif /*
                 #if(ADC_SAR_SAMPLE_MODE !=
                 ADC_SAR_SAMPLE_MODE_FREE_RUNNING)
               */
    }
...
uint32 ADC_SAR_IsEndConversion(uint8 retMode)
{
    ...
    do
    {
      status = ADC_SAR_STATUS_REG;
    } while ((status == 0u) && (retMode == ADC_SAR_WAIT_FOR_RESULT));
...
{
    return (ADC_SAR_finalArray[ADC_SAR_GET_RESULT_INDEX_OFFSET - chan] - ADC_SAR_SAR_shift);
}


/*******************************************************************************
* Function Name: ADC_SAR_GetAdcResult
********************************************************************************
*
* Summary:
...
{
    return (ADC_SAR_SAR_GetResult16());
}
...
{
    ADC_SAR_SAR_SetOffset((int16)offset);
}
...
{
    ADC_SAR_SAR_SetResolution(resolution);
}
...
{
    ADC_SAR_SAR_SetGain((int16)adcGain);
}
...
{
    ADC_SAR_SAR_SetScaledGain(adcGain);
}
...
{
    return ((int32) ADC_SAR_SAR_CountsTo_mVolts(adcCounts));
}
...
{
    return (ADC_SAR_SAR_CountsTo_uVolts(adcCounts));
}
...
{
    return (ADC_SAR_SAR_CountsTo_Volts(adcCounts));
}
